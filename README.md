...
## Who
SellInPublic is created by [Anthony Graddy](https://www.agraddy.com) as a part of my move to focus on sales and marketing and realizing there were not many good resources for sales and marketing from a builder or developer perspective.

## What
SellInPublic is a social network that is a cross between modern social networks and older style forums. 

## When
SellInPublic is available now but it is a very early alphha release. If you would like to request additional features, please submit a new ticket in the DashboardQ Support system. Note that paying customers will receive higher priority.

## Where
SellInPublic is open source and can be installed on any server that supports modern PHP frameworks. The PHP used is very basic and should work with web hosts that support PHP 7.3 and above. It will probably work with older versions of PHP, but I'm not testing or guaranteeing anything for older versions of PHP.
...
These instructions will install SellInPublic on your local computer. The instructions are based on a Unix command line installation but should be easy to apply to other systems.
...
# Clone the repo
git clone https://github.com/dashboardq/sellinpublic.git sellinpublic
cd sellinpublic

# Set up MySQL database
mysql -u root -p --vertical
mysql> CREATE DATABASE sellinpublic;
mysql> exit;
...
# Install dependencies (there are not dependencies at this time so this step may be skipped)
composer install

# (optional) Add a cron that runs every minute
crontab -e
* * * * * cd /home/YOUR_USERNAME/sellinpublic && php ao process > /dev/null 2>&1

# (optional) Or you can manually process the posts if you are just testing the system out.
php ao process
...
4\. **Set up the server.**
  * When setting up the server, Cloudways is going to ask what application you want to install. **Choose "Laravel" for the app** (at the time of this writing, the option was Laravel 8.26.1 which will change in the future). We are not going to use Laravel, but SellInPublic has a similar application structure.
  * It is going to ask you for some additional information for your application. Feel free to use whatever you want, but I would recommend that you **use the info below to set up the app**:
    * Name Your Managed App: SellInPublic
    * Name Your Managed Server: SellInPublic
    * Name Your Project: SellInPublic
...
7\. **Download the source code for SellInPublic**
  * Go to this link, it should automatically start the download process: [SellInPublic Zip](https://github.com/dashboardq/sellinpublic/archive/refs/heads/main.zip).

8\. **Unzip the sellinpublic-main.zip file you just downloaded**
...
11\. **Modify the domain listed in the public/.htaccess file**
  ...
  * Find where it says `sellinpublic.com` and `www.sellinpublic.com` and change that to the domain that you are using.
...
14\. **Using SFTP, upload SellInPublic to your Cloudways server.**
  ...
  * If your SFTP program has a left side and right side pane (using other types of SFTP programs will be slightly different):
    * On the left side, navigate to the directory where you unzipped the sellinpublic-main.zip file containing the SellInPublic code including the `.env.php` file you edited in step 10.
...
15\. **Using SSH, install composer and install the dependencies.** (there are not dependencies at this time so this step may be skipped)
...
16\. **Open the URL with your browser** that Cloudways gave you in the access details section of the Cloudways portal or if you are using a custom domain, use that.
  * When you first open the page, SellInPublic needs to set up the database tables. If you have `DB_INSTALL` set to `true` from step 10, then the first time you load the page, the database tables will be set up.
  * After setting up the database tables, it should then show you the home page of SellInPublic.
...
17\. **Set up the cron job**
  ...
  * In the text field for Advanced, **enter:** `* * * * * cd public_html && php ao process > /dev/null 2>&1`
  * **Click the SAVE CHANGES button**.

18\. **Create an account on the login page** 
  * Visit the URL again in your browser for the new SellInPublic site.
  * Go to the login page and register a new user. 
  * Congratulations! You should now have SellInPublic fully installed on your server. [SellInPublic.com](https://sellinpublic.com) is free to use so you can compare the functionality of the free plan with the functionality on the installation on your Cloudways server.
...
SellInPublic is open source software but it is not currently open contribution software. I'm following the [SQLite model](https://www.sqlite.org/copyright.html) where basically I'm the only one that will be making updates to the code. Feel free to fork, make any changes you want, and use it however you want. If you create a hard fork (meaning you are wanting to take the project in a different direction), I would ask that you rename your project to something else and keep the accreditation in the license file like this:

Right now it says:
```
Copyright (c) 2024 LocationQ
```

If you create a hard fork, please update the license file to something like this:
```
Original Copyright (c) 2024 LocationQ
...
### How often is SellInPublic open source code updated?
...
### Where is the central login and billing code that you are using on SellInPublic and DashboardQ?

If you use the hosted version of SellInPublic, it uses a centralized login and billing system. At this time, I do not have plans to release that code as open source. There are a few other minor differences between the hosted version and the open source version.
...
For now, if you run into any bugs please either add them to the [Github Issue Tracker](https://github.com/dashboardq/sellinpublic/issues) or to the [DashboardQ Support System](https://www.dashboardq.com/support).
...
I'm a strong supporter of the MIT license and am completely aware that it gives someone the right to fully host and compete with my SellInPublic service. My belief is that the majority of the people will want to use the original creator's service and not a copycat service. Please feel free to use, compete, copy, any part of my service. I believe the marketplace is large enough to accomodate multiple companies.
...
If you do launch a service, the one thing I would ask is that you do not confuse the marketplace by claiming to be "SellInPublic", use the SellInPublic logo, or use any trademarked material - please use a different name and logo if you are offering the service publically for others. Feel free to copy any and all other marketing materials in this repository for any startup you may be working on.
...
I know what it is like to be a freelancer and struggle to pay the bills. If this project allows you to charge more to clients, by all means, load this up and start charging more!

The one final note I would make is that launching a profitable software service is very difficult (I've been trying for 17+ years). If you are looking to copy my SellInPublic service, I think it may be harder to achieve success than you realize. If you are wanting to get into building software, I would recommend building a unique tool that solves a problem that you have and/or a problem that you are very familiar with. As you can probably tell, I'm very stubborn and love to compete. If you would like to compete directly, welcome to the competition!
...
Yep. All I ask is that if you are marketing a competing service that you not use the SellInPublic name or logo or make any claims of copyright to material I've written (feel free to use it however you want, just don't claim that you wrote it - you don't need to explicitly reference SellInPublic anywhere publicly or privately).
