...
class APIKey extends Model {
    ...
    public static function validate($username, $password) {
        $data = ao()->db->query('SELECT ak.user_id, ak.api_key FROM usernames un, api_keys ak WHERE un.name = ? AND un.user_id = ak.user_id', $username);

        $pass = false;
        foreach($data as $row) {
            if(password_verify($password, $row['api_key'])) {
                $user_id = $row['user_id'];
                $pass = true;
                break;
            }
